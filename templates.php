...
if($mode == 'jstemplate'){
    $PAGE->requires->js('/mod/data/scripts/ace/ace.js');
}
...
if($mode == 'jstemplate'){
    echo "
        <script>
            var textarea = $('#template');
            var taid = textarea.attr('id');
            textarea.css('display','none');
            console.log(textarea);
            textarea.before('<div id=\"template_div\" style=\"height:300px;width:600px\"></div>');

            var aceeditor = ace.edit('template_div');
            aceeditor.session.setMode('ace/mode/javascript');
            
            $(document).on('DOMSubtreeModified propertychange','.ace_content',function(e){
